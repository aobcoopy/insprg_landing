...
<script>
 /* $(function() {
	$("#panel-heading-user").append('<button id="btnSavesort" type="button" onclick="fn.app.blog.blog.sort()" class="btn btn-info pull-right">Save Sort</button>');
...
$(function(){
	...
	$("#tb_data").DataTable({
		"processing": true,
		"serverSide": true,
		"ajax": "apps/user/db/data_user.php",
		"aoColumns": [
			{ "bSortable": false},
		...
		"createdRow": function ( row, data, index ) {
			...
			s = '';
			s += fn.engine.datatable.button('btn-primary','fa-pencil','fn.user.change('+data[0]+')');
			/*s += ' ';
...
//			$('td', row).eq(7).html(a);
	...
	fn.engine.datatable.add_selectable('tb_data','chk_user');
