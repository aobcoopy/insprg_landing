<?php
	...
	$dbc->Connect();
		...
		//check type file
		$allowed =  array('pdf','PDF','txt','doc','docx');
		...
		else
		{
			...
			$temp = explode(".", $_FILES["file_pdf"]["name"]);
			$newfilename = round(microtime(true)) . '.' . end($temp);
	
			$new_images = "Volums_".$temp[0]."-".$newfilename;
		
			$save_path = "$save_dir/$new_images";
			if(move_uploaded_file($_FILES["file_pdf"]["tmp_name"],$save_dir.'/'.$new_images))
			{
				$save_dir = "/uploads/Volums";
